...
#include "../Host/Inc/CriticalSectionSentry.h"
...
public:
    ...
    std::string GetSPFlashToolLogFolder();
    std::string GetSPFlashToolDumpFileFolder();

    void ResetLogPath(const std::string &new_log_path);
...
private:
    void SetSPFlashToolLogFolder();
    void SetSPFlashToolDumpFileFolder();
    std::string DumpFileWithTimeStamp(void) const;
    void SetPCLogBaseLevel(Level baselevel);
