...
void Logger::SetSPFlashToolLogFolder() {
    ...
    {
        sp_flash_tool_log_path = optionItem.GetStringValue().toStdString();
    }
    else
    {
        sp_flash_tool_log_path = Utils::GetSpecifalFolderPath();
    }
/*#ifdef _WIN32
    ...
    if ( SHGetSpecialFolderPath(NULL, (LPWSTR)szUserPath, CSIDL_COMMON_APPDATA, 0) ) {
        ...
        sp_flash_tool_log_path = subPath + "\\SP_FT_Logs";
    } else {
        sp_flash_tool_log_path = ABS_PATH("SP_FT_Logs");
    }
#else
    sp_flash_tool_log_path = "/tmp/SP_FT_Logs";
#endif
*/
...
#ifdef _WIN32
    return (std::string("\\SP_FT_Dump_") + pimpl->GetTimestamp() );
#else
    return (std::string("/SP_FT_Dump_") + pimpl->GetTimestamp() );
#endif
}
...
void LogFunc::operator ()(const char * format,...)
{
    ...
    Logger::GetLogger()<<Logger::GetLogger().ContextInfo()<<m_func<<"(): ";
