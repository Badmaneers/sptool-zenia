...
public:
    ...
    virtual void OnDAConnected(const DA_REPORT_T&, const std::string &, const int, bool need_collect_dev_info) = 0;
    /*
...
public:
    ...
    typedef void (__stdcall * void_query_device_tracking)(int *p_out);

    Connection(
            _SHARED_PTR<ConnMedia> med,
            _SHARED_PTR<ConnArgBox> box,
            bool storage_life_cycle_check)
        : connected_(0),
          p_media_(med),
          p_arg_box_(box),
          p_inner_conn_(NULL),
          ...
          m_query_device_tracking_cb(NULL)
    {
    }
...
*/
	...
	void setCallBack(void_callback_arg cb_question) {question_cb = cb_question;}
    void setQueryDeviceTracking(void_query_device_tracking query_dev_tracking) { m_query_device_tracking_cb = query_dev_tracking; }

private:
    void NotifyBromConnected(const BOOT_RESULT&, const std::string&) const;
    void NotifyDAConnected(const DA_REPORT_T&, const std::string&, const int usb_status, bool need_collect_dev_info) const;
    void registerDASLACB();
    /*
...
private:
    ...
    void_query_device_tracking m_query_device_tracking_cb;
