...
#include "../UI/src/DASLACallback.h"
...
int Connection::ConnectDA(CONN_DA_END_STAGE stage, _BOOL enableDRAMin1stDA)
{
    ...
    if (1 == connected_)
    {
        ...
        registerDASLACB();
        DASLACB::setFlashToolAPIHandle(this->FTHandle());
        ...
        PHONE_TRACKING_STATUS tracking_status = NOT_SUPPORT_TRACKING_FEATURE;
#ifdef _INTERNAL_PHONE_TRACKING_MODE
        if(!ToolInfo::IsCustomerVer()) {
            tracking_status = FlashTool_Query_Traking_Status(this->FTHandle());
            if (tracking_status == NOT_SUPPORT_TRACKING_FEATURE) {
                LOGI("This platform NOT support phone tracking feature.");
            } else {
                LOGI("This platform support phone tracking feature.");
                if (tracking_status == SUPPORT_TRACKING_FEATURE_CUSTOMER && m_query_device_tracking_cb) {
                    int query_device_tracking = 1;
                    m_query_device_tracking_cb(&query_device_tracking);
                    if (query_device_tracking == 0) {
                        LOGE("user stopped operation due to NOT permitted collecting device information.");
                        THROW_BROM_EXCEPTION(STATUS_DEVICE_TRACKING_STOP, 0)
                    }
                }
            }
        }
        ...
        NotifyDAConnected(da_report(), p_media_.data()->FriendlyName(),
                          p_media_.data()->USBStatus(), tracking_status != NOT_SUPPORT_TRACKING_FEATURE);
        LOGI("DA Connected");
...
void Connection::NotifyDAConnected(const DA_REPORT_T &res,
        ...
        const int usb_status, bool need_collect_dev_info) const
{
    _SHR_OBSERVER_ITERC it = observer_list_.begin();

    while (it != observer_list_.end())
    {
        (*it)->OnDAConnected(res, friend_name, usb_status, need_collect_dev_info);
        ++ it;
    }
}

void Connection::registerDASLACB()
{
    ft_remote_sec_callbacks m_ft_remote_sec_callbacks;
    m_ft_remote_sec_callbacks.server_signature_data = (FCB_SERVER_SIGNATURE_DATA)DASLACB::SERVER_SIGNATURE_DATA;
    m_ft_remote_sec_callbacks.server_verification_flow = (FCB_SERVER_VERIFICATION_FLOW)DASLACB::SERVER_VERIFICATION_FLOW;
    Flashtool_ServerVerificationFlow(m_ft_remote_sec_callbacks);
}
