...
#define FACILITY_ERR_CODE_MASK (0xF)

const static std::string S_RETRY_CABLE_PORT_DEVICE_HINT = "Please retry the following steps:\r\n"
                                                          "1. retry a new cable;\r\n"
                                                          "2. retry a new com port;\r\n"
                                                          "3. retry a new device;\r\n"
                                                          "4. restart the computer then retry again.";
const static std::string S_RETRY_CABLE_PORT_DEVICE__BROM_DOWNLOAD_HINT = "Please retry the following steps:\r\n"
                                                                         "1. retry a new cable;\r\n"
                                                                         "2. retry a new com port;\r\n"
                                                                         "3. retry a new device;\r\n"
                                                                         "4. retry another PC;\r\n"
                                                                         "5. retry pressing download volume down and plug in usb cable.";
const static std::string S_RETRY_MOVE_BATTERY_BROM_DOWNLOAD_HINT = "Please retry the operation:\r\n"
                                                                   "1.remove battery and usb cable;\r\n"
                                                                   "2.press download botton on flash tool;\r\n"
                                                                   "3.pressing download volume down and plug in usb cable and download without battery.";
const static std::string S_RE_FETCH_TOOL_HINT = "Please re-fetch the lastest version and try again.";
const static std::string S_UPGRADE_TOOL_HINT = "Please upgrade to the latest FlashTool then try again.";
const static std::string S_SELECT_VALID_LOAD_HINT = "Please select a valid load or ask for help.";
const static std::string S_CHECK_PRELOADER_HINT = "Please check if preloader exists in load folder.";
const static std::string S_CHECK_LASTEST_TOOL_HINT = "Please check if the FlashTool is the latest version, or ask for help.";
        ...
        unsigned int msp_code)
{
    ...
    if(((error_code>>16 & FACILITY_ERR_CODE_MASK) == 2)
            && (error_code >= STATUS_SEC_IMG_TOO_LARGE))
    {
        char buff[512] = {0};
        FlashTool_GetLastErrorMessage(NULL, buff);
        tipMsg = std::string(buff);
    }
    else
    {
        switch (error_code) {
        case S_COM_PORT_OPEN_FAIL:
            ...
            tipMsg = S_RETRY_CABLE_PORT_DEVICE_HINT;
            break;
        ...
        case S_UNSUPPORTED_VER_OF_BOOTLOADER:
            errMsg = "[MAUI] Boot Loader Image file contains an unsupported version in its header!";
            tipMsg = S_UPGRADE_TOOL_HINT;
            break;

        case S_UNSUPPORTED_VER_OF_DA:
            errMsg = "[DA] DA binary file contains an unsupported version in its header!";
            tipMsg = S_UPGRADE_TOOL_HINT;
            break;
        ...
        case S_INVALID_DA_FILE:
            errMsg = "[User] The DA file does not exists or it is a invalid format.";
            tipMsg = S_RE_FETCH_TOOL_HINT;
            break;
        ...
        case S_DL_SCAT_INCORRECT_FORMAT:
            errMsg = "[MAUI] The scatter file has incorrect format!";
            tipMsg = S_SELECT_VALID_LOAD_HINT;
            break;

        case S_DL_SCAT_ADDR_IS_NOT_WORD_ALIGN:
            errMsg= "[MAUI] The address of some Load Region in the scatter file is\r\n\
                    not word-aligned!";
            tipMsg = S_SELECT_VALID_LOAD_HINT;
            break;

        case S_DL_SCAT_OFFSET_IS_NOT_WORD_ALIGN:
            errMsg = "[MAUI] The offset of some Load Region in the scatter file is\r\n\
                    not word-aligned!";
            tipMsg = S_SELECT_VALID_LOAD_HINT;
            break;

        case S_DL_SCAT_ADDR_IS_NOT_ASCENDING_ORDER:
            errMsg = "[MAUI] The addresses of all Load Region in the scatter file\r\n\
                    are not ascending in order!";
            tipMsg = S_SELECT_VALID_LOAD_HINT;
            break;
        ...
        case S_DL_INVALID_BOOTLOADER:
            errMsg = "[MAUI] The Boot Loader Image file has invalid information in its header!";
            tipMsg = S_SELECT_VALID_LOAD_HINT;
            break;
        ...
        case S_INVALID_SCERT_FILE:
            errMsg = "[User] The certification file is invalid!";
            tipMsg = "Please select a valid certification file or ask for help.";
            break;

        case S_UNSUPPORTED_VER_OF_SCERT_FILE:
            errMsg = "[User] The certification file contains an unsupported version!";
            tipMsg = S_CHECK_LASTEST_TOOL_HINT;
            break;

        case S_DL_SCAT_VERSION_UNSUPPORTED:
            errMsg = "[User] The scatter file contains an unsupported version!";
            tipMsg = S_CHECK_LASTEST_TOOL_HINT;
            break;

        case S_DL_SCAT_PART_NAME_DUPLICATION_ERR:
            errMsg = "[User] The scatter file contains duplicated partition name!";
            tipMsg = S_SELECT_VALID_LOAD_HINT;
            break;
        ...
        case STATUS_SWITCH_USB_EXCEPTION:
            errMsg = "Switch USB COM port fail";
            tipMsg = "Please retry or ask owner for help";
            break;

        case STATUS_PROTOCOL_ERR:
        case STATUS_USB_SCAN_ERR:
            tipMsg = S_RETRY_CABLE_PORT_DEVICE__BROM_DOWNLOAD_HINT;
            break;

        case STATUS_BROM_CMD_STARTCMD_FAIL:
            tipMsg = S_RETRY_MOVE_BATTERY_BROM_DOWNLOAD_HINT;
            break;

        case S_GET_DLL_VER_FAIL:
        case STATUS_DA_VERSION_INCORRECT:
        case STATUS_DA_NOT_FOUND:
            tipMsg = S_RE_FETCH_TOOL_HINT;
            break;

        case STATUS_SCATTER_FILE_INVALID:
            errMsg = "The scatter file format is invalid!";
            tipMsg = S_SELECT_VALID_LOAD_HINT;
            break;

        case STATUS_DA_FILE_INVALID:
            errMsg = "DA file format is invalid!";
            tipMsg = S_RE_FETCH_TOOL_HINT;
            break;

        case STATUS_PRELOADER_INVALID:
            errMsg = "The Preloader file format is invalid!";
            tipMsg = "Please retry another official load.";
            break;

        case STATUS_SEC_CERT_FILE_NOT_FOUND:
            tipMsg = "Please re-select a valid certification file or ask for help.";
            break;

        case STATUS_SEC_AUTH_FILE_NOT_FOUND:
            tipMsg = "Please re-select a valid authentication file or ask for help.";
            break;

        case STATUS_SEC_AUTH_FILE_NEEDED:
            tipMsg = "Please select a valid authentication file or ask for help.";
            break;

        case S_DL_BOOTLOADER_NOT_FOUND:
        case STATUS_BROM_CONNECT_NOT_PL:
        case STATUS_EMI_CONTAINER_FILE_NOT_FOUND:
            tipMsg = S_CHECK_PRELOADER_HINT;
            break;

        case STATUS_SCATTER_FILE_NOT_FOUND:
            tipMsg = "Please retry the following steps:\r\n"
                     "1. check if scatter file exists in load folder;\r\n"
                     "2. select a correct load.";
            break;

        case STATUS_RSC_MAGIC_ERR:
            tipMsg = "Please select a official load or ask for help!";
            break;

        case STATUS_UNKNOWN_SPARSE_CHUNK_TYPE:
            tipMsg = "Please retry a official load, if fail again, please ask RD for help.";
            break;

		case STATUS_STOR_LIFE_EXHAUST:
            errMsg = "The storage in your device has reached end of life, "
                     "please change to use new device or close this feature by the option "
                     "\n\"Storage Life Cycle Check\" in the Option Dialog.";
			break;

		case STATUS_DRAM_SLT_TEST_ERROR:
            errMsg = "Dram SLT failed.\nPlease check Hardware. ";
			break;

		case STATUS_EMI_SLT_TEST_ERROR:
            errMsg = "EMI SLT failed.\nPlease check Hardware. ";
			break;			
...
string ErrorLookup::FlashToolErrorMessage(FLASH_TOOL_RESULT ret) {
    ...
    if (ret == -1) {
            ...
            char buf[256] = {0};
            sprintf(buf,"SP FLASH TOOL ERROR: %s(%d)", FTStatusToString(ret), ret);
            prefix.append(buf);
    }
...
*/
        ...
        case STATUS_SCATTER_FILE_INVALID:
            errMsg = "The scatter file is invalid!";
            tipMsg = "Please check it.";
        case FT_SCATTER_COMBO_PARTSIZE_CHECK_ERR:
            errMsg = "The combo_partisize_check property value is invalid in scatter file!";
            tipMsg = "combo_partisize_check property set to true is used for userdata + mrdump together, please check it.";
...
const char *ErrorLookup::FTStatusToString(FLASH_TOOL_RESULT err_code)
{
    switch(err_code)
    {
    ...
    ENUM_TO_STRING(FT_USER_PRESS_STOP_BUTTON)
    ENUM_TO_STRING(FT_UPGRADE_FILE_CORRUPTED)
    ENUM_TO_STRING(FT_OTP_FILE_NOT_WORD_ALIGN)
    ENUM_TO_STRING(FT_UPGRADE_LAYOUT_CHANGED)
    ENUM_TO_STRING(FT_INVALID_MEM_INFO)
    ENUM_TO_STRING(FT_SAVE_FILE_ERROR)
    ENUM_TO_STRING(FT_LOAD_DA_ERROR)
    ENUM_TO_STRING(FT_GET_DA_INFO_ERROR)
    ENUM_TO_STRING(FT_LOAD_CERT_ERROR)
    ENUM_TO_STRING(FT_LOAD_AUTH_ERROR)
    ENUM_TO_STRING(FT_GET_RESTORE_ITEM_ERROR)
    ENUM_TO_STRING(FT_FILE_FOLDER_IS_NOT_EXIST)
    ENUM_TO_STRING(FT_FILE_IS_NOT_EXIST)
    ENUM_TO_STRING(FT_FLASHTOOL_ERROR_CHKSUM_FAIL)
    ENUM_TO_STRING(FT_FLASHTOOL_INVALIDE_SCATTER)
    ENUM_TO_STRING(FT_LOAD_TRANSLATION_FILE_FAIL)
    ENUM_TO_STRING(FT_RPMB_WRITTEN_BEFORE_DL)
    ENUM_TO_STRING(FT_SCATTER_COMBO_PARTSIZE_CHECK_ERR)
