...
bool DLOnlyCommand::CheckPMTLayoutChange(const QSharedPointer<Connection> &conn)
{
    ...
    int ret_val = info_factory.GetRomList(rom_list);
    if (ret_val != STATUS_OK)
    {
        THROW_APP_EXCEPTION(ret_val, "");
    }
        ...
        rom_it != rom_list.end(); ++rom_it)
    {
        ...
        if(std::string(scat_version) == "V1.2.0" || std::string(scat_version) == "V2.2.0" ) //process combo_partsize_check part first
        {
            ...
            if(rom_it->combo_partsize_check && !process_combo_check_done)
            {
                ...
                if (!next_rom->combo_partsize_check) {
                    THROW_APP_EXCEPTION(FT_SCATTER_COMBO_PARTSIZE_CHECK_ERR, "");
                }
                ...
                U64 combo_pmt_end_addr = pmt->begin_addr + pmt->image_length;
                if (next_pmt != NULL) {
                    combo_pmt_end_addr = next_pmt->begin_addr + next_pmt->image_length;
                }
