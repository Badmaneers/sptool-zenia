...
{
    LOG("checking flashtoollib ret value: %s(%d)", StatusToString(ret), ret);
    if(ret != S_DONE)
    {
                    ...
                    FLASHTOOL_API_HANDLE_T ft_handle)
{
    ...
    for ( int i = 0 ; i < pattern_size ; i++ )
    {
        ...
        if(ret != S_DONE)
        {
            LOG("RAMOnePatternTest failed, error code: %s(%d)", StatusToString(ret), ret);
            break;
        }
    }
...
#define DRAM_FILE_TEST_MAX_SIZE (0x1000000000) //64G
                  ...
                  FLASHTOOL_API_HANDLE_T ft_handle)
{
    ...
    if ( HW_RAM_DRAM == da_report.m_ext_ram_type )
    {
        QString msg = QString("Limitation: current max test DRAM range 0x%1").arg((ulong)DRAM_FILE_TEST_MAX_SIZE, 8, 16, QChar('0'));
        if(da_report.m_ext_ram_size > DRAM_FILE_TEST_MAX_SIZE)
            UpdateUI(msg, Qt::darkRed);
        ...
        if(mt_arg->m_start_addr >= DRAM_FILE_TEST_MAX_SIZE)
        {
            LOGE("addr[0x%llx] >= DRAM_FILE_TEST_MAX_SIZE[0x%llx]", mt_arg->m_start_addr, DRAM_FILE_TEST_MAX_SIZE);
            return STATUS_EXCEED_AVALIABLE_RANGE;
        }
        ...
        if(mt_arg->m_start_addr + mt_arg->m_size > DRAM_FILE_TEST_MAX_SIZE)
        {
            LOGW("addr[0x%llx] + len[0x%llx] > DRAM_FILE_TEST_MAX_SIZE[0x%x]", mt_arg->m_start_addr, mt_arg->m_size, DRAM_FILE_TEST_MAX_SIZE);
            //only test small than DRAM_FILE_TEST_MAX_SIZE
            mt_arg->m_size = DRAM_FILE_TEST_MAX_SIZE - mt_arg->m_start_addr;
            LOGD("change len to 0x%llx", mt_arg->m_size);
        }
    }
    else
    {
