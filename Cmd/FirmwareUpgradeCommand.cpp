...
theClass::FirmwareUpgradeCommand(APKey key):
    ...
    fmt_setting_(NULL),
    dl_setting_(NULL),
    dl_scene_(DOWNLOAD_ONLY),
    is_security_11_(false),
    ...
    RomBaseList &roms_2dl)
{
    ...
    ret = DL_GetPlatformInfo(dl_handle, &info);
    if(S_DONE != ret)
    {
        LOGE("DL_GetPlatformInfo() failed, ret: %s(%d).", StatusToString(ret), ret);
        return ERROR_DATA_LOST;
    }
    std::string bbchip_name = info.m_szBBChipType;
    is_security_11_ = FlashTool_Chip_isOldArch(bbchip_name.c_str());
    //end of set is_security_11_ by conn

    CRomFactory rom_factory(roms_2dl, fmt_setting_->does_format_whole(), is_security_11_);
    ...
    ret = info_factory.GetRomList(rom_list);
    if(S_DONE != ret)
    {
        LOGE("RomInfoFactory::GetRomList() failed, ret: %s(%d).", StatusToString(ret), ret);
        return ERROR_DATA_LOST;
    }
    ...
    std::list<PART_INFO> &roms_2format)
{
    ...
    CRomFactory rom_factory_scatter(roms_scatter, fmt_setting_->does_format_whole(), is_security_11_);
    ERROR_T error = ERROR_OK;
    std::list<ROM_INFO> rom_list_scatter;
    RomInfoFactory info_scatter_factory(key_);
    info_scatter_factory.GetScatterRomList(rom_list_scatter);
        ...
        const QSharedPointer<Connection> &conn)
{
    ...
    if (error == ERROR_OK)
    {
        error = proxy.validate_storage(
                    ...
                    storage.AlignSize(),
                    storage.LogicalSize(),
                    storage.BootPartSize(),
                    storage.UserPartSize());
    }
