...
struct reboot_option
{
   ...
   uint32 bNotDisconnectUSB; //0: disconnect usb when usb download and not reboot; 1: not disconnect usb when usb download and not reboot.
};
...
typedef struct sla_challenge_in_data
{
    uint32 version;
    uint8 random_data[SLA_RANDOM_LENGTH];
    uint8 hrid[SLA_HRID_LENGTH];
    uint8 soc_id[SLA_SOCID_LENGTH];
} sla_challenge_in_data_t;
