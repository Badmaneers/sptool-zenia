...
{
   char8 version[32];
   char8 release_type[128];
   char8 build_date[32];
   char8 change_list[32];
   char8 builder[32];
}dll_info_t;
...
typedef enum device_control_code
{
  ...
  */
    ...
    DEV_DA_READ_INTERNAL_REG32,
    DEV_DA_SET_DYNAMIC_PARTITION_SPACE,
    DEV_DA_GET_SLA_ENABLED_STATUS,
...
struct brom_wdt_reg_addr
{
	...
	brom_wdt_reg_addr(uint32 m, uint32 l, uint32 r, uint32 s)
		:reg_wdt_mode(m),
		 reg_wdt_length(l),
		 reg_wdt_restart(r),
		 reg_wdt_swrst(s)
...
typedef enum {
    NOT_SUPPORT_TRACKING_FEATURE = 0,
    SUPPORT_TRACKING_FEATURE_INTERNAL,
    SUPPORT_TRACKING_FEATURE_CUSTOMER
}PHONE_TRACKING_STATUS;
