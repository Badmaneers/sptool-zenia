...
typedef int (__stdcall *CALLBACK_DOWNLOAD_PROGRESS_INIT)(void *usr_arg, const char *image_name);
...
LIBEXPORT int __stdcall FlashTool_Disconnect_Ex(FLASHTOOL_API_HANDLE_T *p_ft_handle, _BOOL bDisconnectUSB);
LIBEXPORT int __stdcall FlashTool_Disconnect_BROM(FLASHTOOL_API_HANDLE_T  *p_ft_handle);

LIBEXPORT int __stdcall FlashTool_ChangeCOM(FLASHTOOL_API_HANDLE_T *p_ft_handle, unsigned short com_port);
...
typedef struct{
    ...
    _BOOL        m_bNotDisconnectUSB; //0: disconnect usb when usb download and not reboot; 1: not disconnect usb when usb download and not reboot.
...
LIBEXPORT int __stdcall FlashTool_Format_Partition(
	                    FLASHTOOL_API_HANDLE_T  ft_handle, 
...
LIBEXPORT int __stdcall FlashTool_FirmwareUpdate( FLASHTOOL_API_HANDLE_T  ft_handle, const char* fw_filename, const char* fw_buffer, uint32 fw_buffer_len);
...
typedef struct {
	...
	uint16 hpb_pinned_start_idx;
    uint16 hpb_pinned_regions;
}UFS_Config;
...
LIBEXPORT int __stdcall FlashTool_Connect_BROM(unsigned short com_port,
                           const FlashTool_Connect_Arg  *p_arg,
                           FLASHTOOL_API_HANDLE_T  *p_ft_handle,
                           int *p_stopflag,
                           _BOOL bCheckScatter = _TRUE);
...
LIBEXPORT int __stdcall FlashTool_Device_Control(FLASHTOOL_API_HANDLE_T ft_handle,  
...
{
	unsigned char data[SEC_DATA_MAX_LENGTH];
	unsigned int length;
};

struct ft_sec_flow_parameters
{
	struct ft_sec_str_buffer fw_info;
	struct ft_sec_str_buffer sec_policy;
};

typedef BOOL (__stdcall *FCB_SERVER_VERIFICATION_FLOW)(struct ft_sec_flow_parameters* param);
typedef BOOL (__stdcall *FCB_SERVER_SIGNATURE_DATA)(char* pbuf, unsigned int max_buf_length, unsigned int* length);

struct ft_remote_sec_callbacks
{
	FCB_SERVER_VERIFICATION_FLOW server_verification_flow;
	FCB_SERVER_SIGNATURE_DATA server_signature_data;
};

LIBEXPORT void __stdcall Flashtool_ServerVerificationFlow(ft_remote_sec_callbacks cbs);
...
LIBEXPORT bool __stdcall FlashTool_IfLibDAMatch(const char * da_filename);
LIBEXPORT int __stdcall FlashTool_DownloadCert(FLASHTOOL_API_HANDLE_T ft_handle, const char* cert_filename);

LIBEXPORT PHONE_TRACKING_STATUS __stdcall FlashTool_Query_Traking_Status(FLASHTOOL_API_HANDLE_T ft_handle);
