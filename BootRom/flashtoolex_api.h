...
//!   hs: session used to obtain correct storage information from target for combo load, 
//!        if no session in the stage, set to INVALID_HSESSION_VALUE, but the scatter file info maybe incorrect for combo load.
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_get_scatter_info(HSESSION hs, const file_info* scatter, /*IN OUT*/struct scatter_file_info_t* info);
...
//!   count: the wlist count.
//!   callbacks: the callback functions used in device operations.
//!   is_by_sram:  if write data via sram, used to identify if dram issue, default set to false
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_write_data(HSESSION hs, struct op_data_list_t* wlist
                                                   , uint32 count
                                                   , const struct callbacks_struct_t* callbacks
                                                   , bool is_by_sram = false);
...
//! <b>Effects</b>: get runtime-switchable-config cnt in rsc.xml
//! <b>Parameters</b>: rsc_filename:       rsc filefullpath 
//!                                cnt:                     the result para
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_get_rsc_cnt(const char* rsc_filename, uint32* cnt);

//! <b>Effects</b>: get runtime-switchable-config info in rsc.xml
//! <b>Parameters</b>: rsc_filename:       rsc filefullpath 
//!                                rscinfo:                the result para
//!                                rsccnt:                 rsc cnt, get by flashtool_get_rsc_cnt
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_get_rsc_info(const char* rsc_filename, ex_proj_item* rscinfo, uint32 rsccnt);

//! <b>Effects</b>: set user select rsc config to device
//! <b>Parameters</b>: hs:                  the session handle. 
//!					    rsc_filename:       rsc filefullpath 
//!                                ex_da_rsc_field_t:  user select rsc config    
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_set_rsc_info(HSESSION hs, ex_proj_item* da_rsc);

//! <b>Effects</b>: get HRID form device
//! <b>Parameters</b>: hs:                  the session handle. 
//!					    hrid_buf:       out parameter, HRID buffer 
//!                                hrid_buflen:   HRID buffer length    
//!                                log:              true : log on;  false: log off
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_get_hrid(HSESSION hs, uint8* hrid_buf, uint32 hrid_buflen, bool log = false);

//! <b>Effects</b>: check if lib da match before connection
//! <b>Parameters</b>: da_filename:       da filename. 
//!
//! <b>Returns</b> true: match;  false: not match
LIBEX_EXPORT bool LIB_API flashtool_if_lib_da_match(const char8* da_filename);

//! <b>Effects</b>: download certification file to storage
//! <b>Parameters</b>: hs:                the session handle
//!                                cert_filename   certification file full path
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_download_cert(HSESSION hs, const char* cert_filename);

//! <b>Effects</b>: update firmware of storage
//! <b>Parameters</b>: hs:                the session handle
//!                                fw_filename   firmware file full path
//!					     fw_buffer      firmware content buffer
//!					     fw_buffer_len firmware content buffer length
///
//!					    NOTICE: firmware from file or buffer, only one valid.
//!								if use file: set fw_buffer = NULL, fw_buffer_len = 0
//!								if use buffer: set fw_filename =  NULL
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
...
//!
//! <b>Returns</b>:  STATUS_OK if success, or other status indicate specified error.
LIBEX_EXPORT status_t LIB_API flashtool_set_ufs_config(HSESSION hs, struct ex_ufs_config* cfg);
...
//! <b>Effects</b>: check if support internal phone tracking feature.
//! <b>Parameters</b>: hs:       the session handle. 
//!
//! <b>Returns</b> 
//! NOT_SUPPORT_TRACKING_FEATURE: the device not support this feature.
//! SUPPORT_TRACKING_FEATURE_INTERNAL: the device support this feature, and this is a internal device.
//! SUPPORT_TRACKING_FEATURE_CUSTOMER: the device support this feature, and this is a customer device.
LIBEX_EXPORT PHONE_TRACKING_STATUS LIB_API flashtool_query_tracking_status(HSESSION hs);
