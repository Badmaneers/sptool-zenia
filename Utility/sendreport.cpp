...
SendReport::SendReport(QObject *parent, MainWindow *main_window) :
    ...
    main_window_(main_window), bbchipID(0), errorID(0), senderID(0),
    socket(NULL), socketBuf(NULL), fileBuf(NULL), thread_timer_(NULL),
    status(THREAD_STATUS_UNKNOWN),
    currentLogFileIndex(0), packetIndexInFileBuffer(0)
{
    for (int i = 0; i < gConstMaxLogFileNumber; ++i)
    {
        logFiles[i] = NULL;
    }
