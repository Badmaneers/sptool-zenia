...
const static std::map<QString, U32>::value_type INIT_VALUE[]=
{
    ...
    std::map<QString, U32>::value_type("EMMC_BOOT1", EMMC_PART_BOOT1),		
    std::map<QString, U32>::value_type("EMMC_BOOT_2", EMMC_PART_BOOT2),
    std::map<QString, U32>::value_type("EMMC_BOOT2", EMMC_PART_BOOT2),    
...
const static std::map<QString, U32> REGION_MAP(INIT_VALUE, INIT_VALUE+14);
...
std::string EfuseExtraKeyToString(EFUSE_KEY key)
{
    ...
    ENUM_TO_STRING(C_DATA6_LOCK)
    ENUM_TO_STRING(C_DATA7_LOCK)
    ...
    ENUM_TO_STRING(C_DATA_6)
    ENUM_TO_STRING(C_DATA_7)
    ...
    ENUM_TO_STRING(C_CUSTK_LOCK)
    ENUM_TO_STRING(C_TEST_PWD)
    ENUM_TO_STRING(C_TEST_VALID)
    default:
        return "??";
    }
}
...
std::vector<std::string> getLogFile(){
    int i = 0;
    QString log = QString::fromLocal8Bit(Logger::GetLogger().GetSPFlashToolLogFolder().c_str());
    QDir dir(log);
...
U32 Utils::xhtoni(U32 h)
{
    return big_endian() ? h : sw32(h);
}
