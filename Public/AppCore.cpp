                         ...
                         Download_Scene scene)
{
    try{
        ...
        GetImageInfoFromRomList(image_list, rom_info_list, scene);
    }catch(const BromException& e){
        THROW_BROM_EXCEPTION_EX(e);
    }
}

void AppCore::GetImageInfoFromRomList(std::list<ImageInfo> &image_list, const std::list<ROM_INFO> &rom_list, Download_Scene scene)
{
    image_list.clear();
