...
void MainWindowCallback::InstallSignals()
{
	...
	connect(this, SIGNAL(signal_OperationQuestion(int *)), SLOT(slot_OperationQuestion(int *)), Qt::BlockingQueuedConnection);
    connect(this, SIGNAL(signal_query_device_tracking(int*)),
            SLOT(slot_query_device_tracking(int*)), Qt::BlockingQueuedConnection);
    ...
    connect(this,SIGNAL(signal_DownloadFlashInit(void*, const QString&)),
            SLOT(slot_DownloadFlashInit(void*, const QString&)));
    ...
    connect(this, SIGNAL(signal_MemoryTestRepairCallback(int)),
            main_window, SIGNAL(signal_MemoryTestRepairCallback(int)));
...
void __stdcall MainWindowCallback::OperationQuestion(int *p_val)
{
    emit instance_->signal_OperationQuestion(p_val);
}

void MainWindowCallback::QueryDeviceTracking(int *p_out)
{
    emit instance_->signal_query_device_tracking(p_out);
}
...
int __stdcall MainWindowCallback::DownloadFlashInit(void* usr_arg, const char * image_name)
{
    if (NULL != image_name)
    {
        emit instance_->signal_DownloadFlashInit(usr_arg, QString(image_name));
    }
    else
    {
        emit instance_->signal_DownloadFlashInit(usr_arg, QString());
    }
    return 0;
}
...
void MainWindowCallback::MemoryTestRepairCallback(int op_status_code)
{
    emit instance_->signal_MemoryTestRepairCallback(op_status_code);
}
...
void MainWindowCallback::slot_OperationOK()
{
    ...
    if(!main_window->operation_support_auto_polling()
            || !main_window->isAutoPollingEnable()
    ...
    if(!isFinished && !main_window->main_controller()->is_stop_by_user())
    {
        ...
        QString tab_page_name = main_window->ui->tabWidget->tabText(main_window->ui->tabWidget->currentIndex()).trimmed();
        if (tab_page_name == "&Download")
        {
            main_window->startDownload();
        }
        else if (tab_page_name == "&Readback")
        {
            main_window->startReadback();
        }
        else if(tab_page_name == "Form&at")
        {
            main_window->startFormat();
        }
    }
}

void MainWindowCallback::slot_OperationQuestion(int *p_val)
{
	Q_ASSERT(NULL != p_val);
	*p_val = 0;
	int ret = flashtool_message_box(main_window, NULL, QUESTION_MSGBOX, 
		LoadQString(main_window->GetLanguageTag(), IDS_STRING_WARNING),
        LoadQString(main_window->GetLanguageTag(), IDS_STRING_STOR_LIFE_WARN_REMINDER),
		LoadQString(main_window->GetLanguageTag(), IDS_STRING_CONTINUE),
		LoadQString(main_window->GetLanguageTag(), IDS_STRING_CANCEL));
...
void MainWindowCallback::slot_query_device_tracking(int *p_out)
{
    Q_ASSERT(NULL != p_out);
...
#define MTK_VID "vid_0e8d"
    ...
    *p_out = 1;
    ...
    if (ToolInfo::IsCustomerVer() || !main_window->main_controller()->networkOnline()) {
        return ;
    }

    QString vid = QString::fromStdString(pid_vid).split("&").at(0);

    if (vid.toLower() == MTK_VID) return;
    LOG("mismatch vendor id: %s", vid.toStdString().c_str());
*/

    *p_out = 1;
    int ret = flashtool_message_box(NULL, NULL, QUESTION_MSGBOX,
        tr("Warning"),
...
void MainWindowCallback::slot_DownloadFlashInit(void* usr_arg, const QString &image_name)
{
    Q_UNUSED(usr_arg);
    if(!main_window->main_controller()->isStop())
    {
        imageName = image_name.trimmed();
        main_window->ui->progressBar->setStyleSheet(PColorYellow);
        main_window->ui->progressBar->setValue(0);
        if (!imageName.isEmpty())
        {
            main_window->ui->progressBar->setFormat(tr("[%1] Download Flash %2%").arg(imageName).arg(0));
        }
        else
        {
        ...
        void *usr_arg)
{
    ...
    if(!main_window->main_controller()->isStop())
    {
        ...
        if (!imageName.isEmpty())
        {
            main_window->ui->progressBar->setFormat(tr("[%1] Download Flash %2%").arg(imageName).arg(finished_percentage));
        }
        else
        {
