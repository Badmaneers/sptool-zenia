    ...
    mCurLocationRow(-1)
{
    ...
    ui_->tableWidget->setAcceptDrops(true);
    ui_->tableWidget->installEventFilter(this);
    ...
    QString scatter_tooltip("Please select the scatter file normally named MTXXXX__Android_scatter.txt/.xml in the load path.");
    ui_->pushButton_scatterLoading->setToolTip(scatter_tooltip);
    ui_->comboBox_scatterFilePath->setToolTip(scatter_tooltip);
}
...
void DownloadWidget::StartDownload()
{
    ...
    if(main_window_->autoRebootAfterDL())
...
void DownloadWidget::dragEnterEventHandler(QDragEnterEvent *event)
{
    if (event->mimeData()->hasUrls()) {
        ...
        if (file_info.suffix().toLower() == "xml" || file_info.suffix().toLower() == "txt") {
...
void DownloadWidget::choose_rom_file(int row)
{
    ...
    if(!new_file.isEmpty())
    {
        ...
        // process of preloader / loader_ext1(loader_ext_a) / loader_ext2(loader_ext_b)
        ...
        if(IsPreloaderPartition(partition_name))
        {
            ...
            if (loader_ext1_index == -1 && loader_ext2_index == -1 && loader_ext1_visible_index == -1
                    && loader_ext2_visible_index == -1)
            {
                return;
            }
        ...
        //end of process of preloader / loader_ext1(loader_ext_a) / loader_ext2(loader_ext_b)
    }
}
...
{
    return (partition_name == "preloader" || partition_name == "loader_ext1" || partition_name == "loader_ext2"
            || partition_name == "loader_ext_a" || partition_name == "loader_ext_b");
}
    ...
    *ext2_index = -1;
    ...
    for(int i = 0; i<row_cnt; i++)
    {
        ...
        if(tableItem)
        {
            ...
            if(item_part_name == "loader_ext1" || item_part_name == "loader_ext_a")
            {
                *ext1_index = i;
            ...
            if(item_part_name == "loader_ext2" || item_part_name == "loader_ext_b")
            {
                *ext2_index = i;
...
void DownloadWidget::Enable_preloaders_partition(bool enable)
{
    ...
    // fix bug: download loader_ext_b failed when partition[loader_ext_b] no need to download in scatter config.
    if(pl_index != -1 && pl_visible_index != -1) {
    ...
    if(loader_ext1_index != -1 && loader_ext1_visible_index != -1) {
    ...
    if(loader_ext2_index != -1 && loader_ext2_visible_index != -1) {
...
void DownloadWidget::on_comboBox_authFilePath_activated(int index)
{
    ...
    {
        IniItem item("history.ini", "RecentOpenFile", "lastAuthDir");
        item.SaveStringValue(QDir::toNativeSeparators(authFile_historyList_[index]));
...
void DownloadWidget::on_comboBox_certFilePath_activated(int index)
{
    ...
    {
        IniItem item("history.ini", "RecentOpenFile", "lastCertDir");
        item.SaveStringValue(QDir::toNativeSeparators(certFile_historyList_[index]));
