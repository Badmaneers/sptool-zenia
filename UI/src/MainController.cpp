    ...
    m_network_online(false)
{
    ...
    connect(this, SIGNAL(signal_GetDAReport(const DA_REPORT_T*, const std::string&, const int, bool)),
            main_window, SLOT(slot_GetDAReport(const DA_REPORT_T*,const std::string&, const int, bool)));
...
bool MainController::LoadDA(const QString& file_name)
{
    ...
    {
        bool check = false;
        if(main_window->LibDAMatchChecked())
            check = true;

        if(check && !FlashTool_IfLibDAMatch(file_name.toLocal8Bit().constData()))
        {
            LOGE("Error: lib DA NOT match!");
            QString err_msg = "LIB DA not match, please re-select DA or ask for help";
            flashtool_message_box(main_window,
                                  main_window->getConnectButton(),
                                  CRITICAL_MSGBOX,
...
unsigned int MainController::QueryROMIndex(std::string partition_name)
{
    try{
        //get ROM index ...
        std::list<ImageInfo> image_list;
...
//for pl, ext1, ext2 always process same time, so query once to imporve performance
void MainController::QueryPreloadersIndex(int* pl_index, int* ext1_index, int* ext2_index)
{
    try{
        std::list<ImageInfo> image_list;
        GetImageInfoList(image_list);
        ...
        *ext2_index = -1;
            ...
            ++it)
        {
            ...
            if(it->name == "loader_ext1" || it->name == "loader_ext_a")
            {
                *ext1_index = it->index;
            ...
            if(it->name == "loader_ext2" || it->name == "loader_ext_b")
            {
                *ext2_index = it->index;
...
void MainController::StartExecuting(ICallback *callback)
{
    Q_ASSERT(background_worker != NULL);
    is_stop_by_user_ = false;
    background_worker->setQueryDeviceTracking(m_network_online ? main_window->main_callbacks()->QueryDeviceTracking : NULL);
...
void MainController::CreateNewThread()
{
    ...
    background_worker = new BackgroundWorker(session_id,app_core,this,
							...
							main_window->main_callbacks()->OperationQuestion);
...
{
    emit signal_GetDAReport(&da_report, friend_name, usb_status, need_collect_dev_info);
}
