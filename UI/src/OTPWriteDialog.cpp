...
void OTPWriteDialog::onScatterChanged(bool showRegion, bool scatter_ver2)
{
    ui->comboBox_region->clear();

    if (scatter_ver2)
    {
        ui->comboBox_region->addItems(mUfsEmmcRegionList);
...
{
    OTP_OPERATION oper = OTP_OPERATION_END;

    if(ui->rb_write_only->isChecked())
    {
        oper = OTP_WRITE;
    }
    else if(ui->rb_otp_wl->isChecked())
    {
        oper = OTP_WRITE_AND_LOCK;
    }
    else if(ui->rb_lock_only->isChecked())
    {
        oper = OTP_LOCK;
    }
