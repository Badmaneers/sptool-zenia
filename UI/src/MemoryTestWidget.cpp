    ...
    m_dram_repair_status_code(STATUS_DRAM_REPAIR_COMPLETE)
{
    ...
    connect(main_window_, SIGNAL(signal_MemoryTestRepairCallback(int)),
...
void MemoryTestWidget::LockOnUI()
{
    ...
    ui_->toolButton_repair->setEnabled(false);
...
void MemoryTestWidget::DoFinished()
{
    ...
    ui_->toolButton_repair->setEnabled(true);
...
void MemoryTestWidget::OnScatterChanged(bool showRegion, bool scatter_ver2)
{
...
void MemoryTestWidget::on_toolButton_repair_clicked()
{
    clearUIText();
    if(main_window_->ValidateBeforeMemoryTest(false))
    {
        main_window_->main_controller()->SetPlatformSetting();
        main_window_->main_controller()->SetConnSetting(main_window_->CreateConnSetting());
        main_window_->main_controller()->QueueAJob(main_window_->CreateDRAMRepairSetting());
        main_window_->main_controller()->QueueAJob(main_window_->CreateWatchDogSetting());
        main_window_->main_controller()->StartExecuting(
                        new SimpleCallback<MemoryTestWidget>(this,&MemoryTestWidget::OnDRAMRepairFinished));
        main_window_->LockOnUI();
        main_window_->GetOkDialog()->setWindowTitle(LoadQString(main_window_->GetLanguageTag(), IDS_STRING_DRAM_REPAIR_COMPLETE));
    }
}

void MemoryTestWidget::slot_dram_repair_finished()
{
    if (m_dram_repair_status_code == STATUS_OK)
    {
        flashtool_message_box(this, 0, INFORMATION_MSGBOX,
                              LoadQString(main_window_->GetLanguageTag(), IDS_STRING_DRAM_REPAIR_TITLE),
                              LoadQString(main_window_->GetLanguageTag(), IDS_STRING_DRAM_REPAIR_EMPTY),
                              LoadQString(main_window_->GetLanguageTag(), IDS_STRING_OK));
    }
...
{
    ui_->toolButton_repair->setVisible(isShow);
}
...
QSharedPointer<APCore::DRAMRepairSetting> MemoryTestWidget::CreateDRAMRepairSetting()
{
...
void MemoryTestWidget::slot_MemoryTestRepairCallback(int op_status_code)
{
    m_dram_repair_status_code = op_status_code;
}
...
void MemoryTestWidget::OnDRAMRepairFinished()
{
