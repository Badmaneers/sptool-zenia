...
void MainWindow::CreateWidget()
{
    ...
    fw_widget_ = new FW(ui->tabWidget, this);
...
void MainWindow::CreateConnect()
{
    ...
    connect(async_updater, SIGNAL(signal_update_failed(AsyncUpdater::UpdateFailReason)),
...
void MainWindow::SetInitState()
{
    ...
    UpdateWindowTitle();
    LogOperationMgr::LogSwitch(mLog_on);
}
...
MainWindow::~MainWindow()
{
    ...
    if(scatter_observer_)
    {
        delete scatter_observer_;
        scatter_observer_ = NULL;
    }
    if(assistant_)
    {
        delete assistant_;
    ...
    if(main_controller_)
    {
        ...
        main_controller_ = NULL;
    }
    if(main_callbacks_)
    {
        delete main_callbacks_;
        main_callbacks_ = NULL;
    }
    if(send_report_)
    {
...
QSharedPointer<APCore::FormatSetting> MainWindow::CreateFormatSetting()
{
    ...
    bool is_scatter_ver2 = IsScatterVer2();
    setting->set_is_scatter_ver2(is_scatter_ver2);
...
QSharedPointer<APCore::ComboFormatSetting> MainWindow::CreateComboFormatSetting(DL_SCATTER_TYPE type)
{
    ...
    setting->set_storage_type(main_controller_->GetPlatformSetting()->getFlashToolStorageConfig().GetStorageType());
    setting->set_is_scatter_ver2(this->IsScatterVer2());
...
QSharedPointer<APCore::FirmwareUpgradeSetting> MainWindow::CreateFirmwareUpgradeSetting()
{
    ...
    setting->set_combo_format(format_widget->get_is_com_dl());
...
QSharedPointer<APCore::RSCSetting> MainWindow::CreateRSCSetting()
{
    QSharedPointer<APCore::RSCSetting> setting(new APCore::RSCSetting());

    std::string rscProjName = download_widget->GetRSCProjectName();
    setting->setRSCProjectName(rscProjName);
    int index = download_widget->GetRSCIndex(rscProjName);
    if(index == -1)
    {
        flashtool_message_box(this,
                              getConnectButton(),
                              CRITICAL_MSGBOX,
                              "Smart Phone Flash Tool",
                              "not found index for this rsc item",
                              "OK");
    }
...
QSharedPointer<APCore::ReadbackSetting> MainWindow::CreateReadbackSetting()
{
    ...
    setting->set_storage_type(main_controller_->GetPlatformSetting()->getFlashToolStorageConfig().GetStorageType());
    setting->set_is_scatter_ver2(this->IsScatterVer2());
    setting->set_is_sram_rb(readback_widget->IsSRAMReadback());
...
QSharedPointer<APCore::DRAMRepairSetting> MainWindow::CreateDRAMRepairSetting()
{
    QSharedPointer<APCore::DRAMRepairSetting> setting = memtest_widget->CreateDRAMRepairSetting();
    setting->set_cb_repair(main_callbacks_->MemoryTestRepairCallback);
    return setting;
}
...
QSharedPointer<APCore::CertDLSetting> MainWindow::CreateCertDLSetting()
{
    ...
    setting->SetDLToStorage(download_widget->GetCertDLToStorage());
...
QSharedPointer<APCore::FWSetting> MainWindow::CreateFWSetting()
{
   QSharedPointer<APCore::FWSetting> setting = fw_widget_->CreateFWSetting();
   return setting;
}
...
QSharedPointer<APCore::LogInfoSetting> MainWindow::CreateLogInfoSetting()
{
    return option_dialog->CreateLogInfoSetting();
}
...
QSharedPointer<ConsoleMode::GeneralSetting> MainWindow::CreateGeneralSetting()
{
    ...
    gen_setting->SetLogInfoSetting(CreateLogInfoSetting());
...
void MainWindow::startReadback()
{
    emit signal_start_readback();
}

void MainWindow::startFormat()
{
    emit signal_start_format();
}
...
#ifdef _WIN32
void MainWindow::slot_update_failed(AsyncUpdater::UpdateFailReason failReason)
{
    switch (failReason)
    {
    case AsyncUpdater::lack_of_update_files:
        flashtool_message_box(this, 0, WARNING_MSGBOX,
                              LoadQString(this->GetLanguageTag(), IDS_STRING_UPDATE_TITLE),
                              LoadQString(this->GetLanguageTag(), IDS_STRING_LACKING_OF_UPDATE_FILES),
                              LoadQString(this->GetLanguageTag(), IDS_STRING_OK));
        break;
    case AsyncUpdater::start_update_exe_fail:
        flashtool_message_box(this, 0, WARNING_MSGBOX,
                              LoadQString(this->GetLanguageTag(), IDS_STRING_UPDATE_TITLE),
                              LoadQString(this->GetLanguageTag(), IDS_STRING_START_UPDATE_TOOL_FAILED),
                              LoadQString(this->GetLanguageTag(), IDS_STRING_OK));
        break;
    default:
        flashtool_message_box(this, 0, WARNING_MSGBOX,
                              LoadQString(this->GetLanguageTag(), IDS_STRING_UPDATE_TITLE),
                              LoadQString(this->GetLanguageTag(), IDS_STRING_UPDATE_FAILED),
                              LoadQString(this->GetLanguageTag(), IDS_STRING_OK));
        break;
    }
}
...
void MainWindow::setChipInfo(ChipInfoWidget *widget, const DA_REPORT_T *p_da_report)
{
   ...
   main_controller()->GetScatterHeadInfo(&info);
   QString chip_name = info.platform;
                                  ...
                                  const int usb_status, bool need_collect_dev_info)
{
    ...
    AfterDAConnected(p_da_report, need_collect_dev_info);
}
...
{
    if (IsDALoad(false) && IsScatterFileLoad(false))
    {
        int readback_row_count = readback_widget->GetRBCount();
        if (readback_row_count > 0)
        {
            QString file_name("readback_ui_bak.xml");
            this->SaveReadBackXMLFile(file_name, false);
        }
    }
}
...
void MainWindow::slot_show_err(int err_code, const std::string& err)
{
    if(err_code < 0)
    {
        Platform platform = main_controller()->GetPlatformSetting()->getPlatformConfig();
    ...
    else
    {
        ...
        {
            content = ErrorLookup::FTStatusToString(static_cast<FLASH_TOOL_RESULT>(err_code));
        }
        else
        {
        ...
        ShowHelpContents(this, err.c_str(), content, true);
    }
...
void MainWindow::CleanChipAndStorageInfo()
{
    if(nor_widget_ != NULL)
    {
        delete nor_widget_;
...
void MainWindow::SaveReadBackXMLFile(const QString &fileName, bool full_general_setting)
{
    QString file_name = QDir::toNativeSeparators(fileName);
    ...
    config.SaveFile(file_name.toLocal8Bit().constData(), full_general_setting);
}

void MainWindow::collectDeviceInfo(const DA_REPORT_T *p_da_report)
{
    Q_ASSERT(p_da_report);

    if (!main_controller_->networkOnline()) {
        return ;
    }

    QString device_id;
    if (p_da_report->m_emmc_ret == S_DONE)
    {
#define MMC_CID_CNT sizeof(p_da_report->m_emmc_cid) / sizeof(p_da_report->m_emmc_cid[0])
        for (int i = 0; i < MMC_CID_CNT; ++i) {
            U32 cid = Utils::xhtoni(p_da_report->m_emmc_cid[i]);
            device_id.append(QString("%1").arg(cid, 0, 16));
        }
        //device_id = QString("%1%2%3%4").arg(p_da_report->m_emmc_cid[0], 0, 16).arg(p_da_report->m_emmc_cid[1], 0, 16)
                //.arg(p_da_report->m_emmc_cid[2], 0, 16).arg(p_da_report->m_emmc_cid[3], 0, 16);
    }
    else if (p_da_report->m_ufs_ret == S_DONE)
    {
        device_id = QString::fromStdString((char *)p_da_report->m_ufs_sn);
    }
...
void MainWindow::AfterDAConnected(const DA_REPORT_T *p_da_report, bool need_collect_dev_info)
{
    if (IsScatterVer2())
    {
        HW_StorageType_E storage_type = getDeviceStorageType(p_da_report);
        Q_ASSERT(storage_type != HW_STORAGE_NONE && storage_type != HW_STORAGE_TYPE_END);
        UpdateStorageLabel(storage_type);
    }
#ifdef _INTERNAL_PHONE_TRACKING_MODE
    if (need_collect_dev_info && !ToolInfo::IsCustomerVer()) {
        collectDeviceInfo(p_da_report);
    }
#endif
}

void MainWindow::UpdateStorageLabel(HW_StorageType_E storage_type)
{
    std::string storage_name = main_controller()->GetPlatformSetting()->\
            getFlashToolStorageConfig().QueryStorageNameByStorageType(storage_type);
    ui->label_storage->setText(QString(storage_name.c_str()));
}

bool MainWindow::IsScatterVer2() const
{
    SCATTER_Head_Info header_info;
    main_controller()->GetScatterHeadInfo(&header_info);
    bool scatter_ver2 = QString(header_info.version).indexOf("V2", 0, Qt::CaseInsensitive) != -1;
    return scatter_ver2;
}

void MainWindow::setStorageLabel(const QString &text)
{
    ui->label_storage->setText(text);
}

QString MainWindow::storageLabelText() const
{
    return ui->label_storage->text();
}

bool MainWindow::LibDAMatchChecked()
{
    return option_dialog->LibDAMatchChecked();
}
...
bool MainWindow::autoRebootAfterDL() const
{
    return this->option_dialog->autoRebootAfterDL();
}
...
void MainWindow::slot_language_changed(int index)
{
    ...
    IniItem item("option.ini", "Conn", "USBHighSpeed");
    if(item.GetBooleanValue())
...
void MainWindow::toggleLogging()
{
    ...
    logSwitch();
    option_dialog->updateToolLogStatus(mLog_on);
}
...
void MainWindow::toggleEnableAdvanceMode()
{
    ...
    option_dialog->ShowPhysicalFormatItem(isAdvanceMode && storage_type == HW_STORAGE_NAND);
...
#ifdef _INTERNAL_PHONE_TRACKING_MODE
    if (!ToolInfo::IsCustomerVer()) {
        ui->actionNetwork_Test->setVisible(isAdvanceMode);
    }
#endif

    if(isAdvanceMode)
    {
        tab_widgets.push_back(fw_widget_);
        fw_widget_->Attach();
        fw_widget_->SetTabLabel(ui->tabWidget, ui->tabWidget->indexOf(fw_widget_));
    }
    else
    {
        tab_widgets.remove(fw_widget_);
        fw_widget_->Detach();
    }
    ...
    {
        memtest_widget->ShowEMMCTest(isAdvanceMode && storage_type == HW_STORAGE_EMMC);
        memtest_widget->ShowNANDTest(isAdvanceMode && storage_type == HW_STORAGE_NAND);
    }
    memtest_widget->ShowDRAMRepairBtn(isAdvanceMode);

    /*
    memtest_widget->ShowUFSTest(isAdvanceMode && storage_type == HW_STORAGE_UFS);
    */
...
void MainWindow::toggleSCIDownloadWidget()
{
    ...
    if(mSCIDownload_on)
    {
        if(NULL == encry_dialog)
        {
            encry_dialog = new EncryDialog(this);
        }
...
void MainWindow::on_actionExportDownload_triggered()
{
    ...
    if(!file_name.isEmpty())
    {
        ...
        if (download_widget->IsRSCEnabled())
        {
            config.vAddCommandSetting(this->CreateRSCSetting());
        }
        config.SaveFile(file_name.toLocal8Bit().constData());
    }
}
...
void MainWindow::slot_current_changed(int index)
{
    QString tab_page_name = ui->tabWidget->tabText(index).trimmed();
    if (tab_page_name == "&Readback")
    {
        readback_widget->AutoLoadReadBackSetting();
    }
}
...
{
    NetworkThread *network_thread = new NetworkThread(QString());
    connect(network_thread, SIGNAL(signal_network_connected(bool)), this, SLOT(slot_show_network_connected(bool)));
    connect(network_thread, SIGNAL(finished()), network_thread, SLOT(deleteLater()));
    network_thread->start();
}

void MainWindow::slot_network_connected(bool connected)
{
    this->main_controller_->setNetworkOnline(connected);
}

void MainWindow::slot_show_network_connected(bool connected)
{
    main_controller_->setNetworkOnline(connected);
    if (connected) {
        flashtool_message_box(this, NULL, INFORMATION_MSGBOX, tr("Smart Phone Flash Tool"), "Connection successed", tr("OK"));
    } else {
        flashtool_message_box(this, NULL, WARNING_MSGBOX, tr("Smart Phone Flash Tool"), "Connection failed", tr("OK"));
    }
}
...
{
    int rbCount = readback_widget->GetRBCount();
    if (rbCount == 0)
    {
        flashtool_message_box(this,
                              0,
                              INFORMATION_MSGBOX,
                              "Save File",
                              LoadQString(language_tag, IDS_STRING_ADDRB_BEFORE_EXPORT),
                              LoadQString(language_tag, IDS_STRING_OK));
        return;
    }
    QString file_name = FileDialogHelper::getSaveFileName(
    ...
    if(!file_name.isEmpty())
    {
        SaveReadBackXMLFile(file_name, true);
    }
}
...
void MainWindow::on_actionSecurity_Mode_triggered()
{
    ...
    if(!msecurity_on)
    {
        download_widget->LoadCertFile("");//unload cert file
    }
...
void MainWindow::ShowHelpContents(QWidget *parent, const QString &errMsg, const QString &page, bool report)
{
    Platform platform = main_controller()->GetPlatformSetting()->getPlatformConfig();
    std::string sOwnerName = platform.getOwnerName();

    flashtool_message_box(parent,
                          this->getConnectButton(),
                          WARNING_MSGBOX,
                          LoadQString(language_tag, IDS_STRING_TOOL_NAME),
                          errMsg,
...
void MainWindow::ShowHelpContents(QWidget* parent, int errID, const QString &page, bool report)
{
...
void MainWindow::on_action_SoftwareUpdate_triggered()
{
    ...
    checkupdate_dialog->CheckUpdate();
...
{
    QString path = QString::fromLocal8Bit(Logger::GetLogger().GetSPFlashToolLogFolder().c_str());
...
bool MainWindow::IsScatterFileLoad(bool showPrompt)
{
    return download_widget->IsScatterFileLoad(showPrompt);
}

bool MainWindow::IsDALoad(bool showPrompt)
{
...
bool MainWindow::ValidateBeforeMemoryTest(bool bCheckMemorySetting /*= true*/)
{
    if(!IsDALoad() || !IsScatterFileLoad())
        return false;

    if(bCheckMemorySetting && memtest_widget->isSelectNothing())
    {
...
{
    return IsDALoad() && IsScatterFileLoad();
}
...
{
    disconnect(ui->tabWidget, SIGNAL(currentChanged(int)), this, SLOT(slot_current_changed(int)));
    if(lockui_on)
    {
        ...
        int promptbutton = flashtool_message_box(this,
                                             this->getConnectButton(),
                                             QUESTION_MSGBOX,
                                             title,
                                             info,
...
{
    if(obj == connectButton && event->type() == QEvent::MouseButtonPress)
    {
        LOGD("grab window");
        grabWindow();
    }
