...
#define DOWNLOAD_SECTION_NAME "Download"
#define AUTO_REBOOT_AFTER_DL_NAME "AutoRebootAfterDL"
    ...
    m_workerThread(NULL)
{
    ...
    ui->lineEdit_logPath->setText(QString::fromLocal8Bit(Logger::GetLogger().GetSPFlashToolLogFolder().c_str()));
...
void OptionDialog::InitSettings()
{
    ...
    if (item.hasKey(AUTO_REBOOT_AFTER_DL_NAME)) {
        item.SetItemName(AUTO_REBOOT_AFTER_DL_NAME);
        ui->checkBox_auto_reboot->setChecked(item.GetBooleanValue());
    } else {
        ui->checkBox_auto_reboot->setChecked(!ToolInfo::IsCustomerVer());
    }
...
void OptionDialog::LoadGeneralSettings()
{
    ...
    ui->groupBox_enable_tool_log->setChecked(item.GetBooleanValue());
...
void OptionDialog::SaveGeneralSettings()
{
    ...
    item.SaveBooleanValue(ui->groupBox_enable_tool_log->isChecked());
    ...
    item.SetSectionName(DOWNLOAD_SECTION_NAME);
    item.SetItemName(AUTO_REBOOT_AFTER_DL_NAME);
    item.SaveBooleanValue(ui->checkBox_auto_reboot->isChecked());
...
QSharedPointer<APCore::LogInfoSetting> OptionDialog::CreateLogInfoSetting()
{
    ...
    log_setting->setLogOn(ui->groupBox_enable_tool_log->isChecked());
...
bool OptionDialog::autoRebootAfterDL() const
{
    return ui->checkBox_auto_reboot->isChecked();
}
...
void OptionDialog::on_btn_logPath_clicked()
{
    ...
    if (!sSelectedLogPath.isEmpty() && sCurrentLogPath != sSelectedLogPath)
    {
        ...
        IniItem item("option.ini", "Trace", "LogPath");
        item.SaveStringValue(sSelectedLogPath);
        LogOperationMgr::ResetLogPath(sSelectedLogPath.toStdString());
    }
}
