...
QSharedPointer<APCore::UFSConfigSetting> FW::CreateUFSConfigSetting()
{
    ...
    uint16 hpb_pinned_start_idx = (ui->lineEdit_HPB_Pinned_Region_Start_Index->text().toUShort(0, 16));
    setting->set_hpb_pinned_start_idx(hpb_pinned_start_idx);
    uint16 hpb_pinned_regions = (ui->lineEdit_Pinned_Region_Count->text().toUShort(0, 16));
    setting->set_hpb_pinned_regions(hpb_pinned_regions);
