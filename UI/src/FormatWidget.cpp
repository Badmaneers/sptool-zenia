    ...
    is_combo_dl_(false)
{
    ...
    mUfsEmmcCommonRegionList << "BOOT_1" << "BOOT_2" << "USER";
...
bool FormatWidget::IsCurPlatformSupportFmtOthers() const
{
    bool bSupportFmtOthers(true);
    SCATTER_Head_Info scatter_header_info;
    main_window_->main_controller()->GetScatterHeadInfo(&scatter_header_info);
    std::string sPlatformName = std::string(scatter_header_info.platform);
    for (unsigned short i = 0; i < sizeof(unsupport_fmt_other_chips) / sizeof(std::string); ++i)
    {
        if (unsupport_fmt_other_chips[i].compare(sPlatformName) == 0)
        {
            bSupportFmtOthers = false;
            break;
        }
    }
    return bSupportFmtOthers;
}
...
void FormatWidget::UpdateUI()
{
    ...
    if(platform_changed_)
    {
        ...
        bool bSupportAutoFmtOthers = IsCurPlatformSupportFmtOthers();
        ui_->radioButton_fmt_all->setVisible(bSupportAutoFmtAll);
        ui_->radioButton_fmt_all->setChecked(bSupportAutoFmtAll || !bSupportAutoFmtOthers);
        ui_->radioButton_fmt_others->setVisible(bSupportAutoFmtOthers);
        ui_->radioButton_fmt_others->setChecked(!bSupportAutoFmtAll && bSupportAutoFmtOthers);
...
void FormatWidget::OnScatterChanged(bool showRegion, bool scatter_ver2)
{
    ...
    if(showRegion)
    {
        ...
        if (scatter_ver2)
        {
            ui_->comboBox_region->addItems(mUfsEmmcCommonRegionList);
