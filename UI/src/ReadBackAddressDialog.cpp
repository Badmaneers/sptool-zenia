...
ReadBackAddressDialog::ReadBackAddressDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ReadBackAddressDialog), storage_type_(HW_STORAGE_TYPE_END)
{
    ui->setupUi(this);
    setModal(true);
...
void ReadBackAddressDialog::OnScatterChanged(bool showRegion, bool scatter_ver2)
{
    if(showRegion)
    {
        ...
        if (scatter_ver2)
        {
            ui->comboBox_region->addItems(mUfsEmmcCommonRegionList);
        }
        else if(HW_STORAGE_EMMC == storage_type_)
