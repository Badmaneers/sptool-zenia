...
public:
    ...
    unsigned int QueryROMIndex(std::string partition_name);

    void QueryPreloadersIndex(int* pl_index, int* ext1_index, int* ext2_index);
    ...
    void get_da_report(const DA_REPORT_T &da_report, const std::string&, const int, bool need_collect_dev_info);
    void get_brom_result(const BOOT_RESULT &brom_result, const std::string&);
    void set_loop_dl_ok(bool is_ok);
    inline void setNetworkOnline(bool online) { m_network_online = online; }
    inline bool networkOnline() const { return m_network_online; }
...
signals:
    ...
    void signal_show_ok();
    void signal_GetDAReport(const DA_REPORT_T *p_da_report, const std::string& friend_name,
