...
BackgroundWorker::BackgroundWorker(const APKey session_id, AppCore* ap_core, MainController *control,
                                   void_callback cb, void_callback cb_ok, void_callback_arg cb_question, 
								   QObject *parent) :
    QThread(parent),session_id_(session_id), ap_core_(ap_core),
    current_cb(NULL),conn_setting_(NULL), m_query_device_tracking_cb(NULL),
    platform_setting_(new APCore::PlatformSetting), stop_flag(AP_STOP),finished_cb(cb), ok_cb(cb_ok), 
	question_cb(cb_question), control_(control)
{
...
void BackgroundWorker::setQueryDeviceTracking(BackgroundWorker::void_query_device_tracking query_device_tracking)
{
...
void BackgroundWorker::run()
{
    ...
    try
    {
        ...
        conn->setQueryDeviceTracking(m_query_device_tracking_cb);
    ...
    catch(const BromException& brom_exp)
    {
        ...
        else if (brom_exp.err_code() == STATUS_OK) //specially for DRAMRepairCommand
        {
            LOGI("DRAM repair status: DRAM reapair address is NOT written!");
        }
        else if (brom_exp.err_code() == STATUS_STOR_LIFE_WARN || brom_exp.err_code() == STATUS_DEVICE_TRACKING_STOP) //has dealed with in DA connection function, reset UI throw exception.
		{
            // do nothing, log info has been written in DA connection function.
		}
        else
        {
